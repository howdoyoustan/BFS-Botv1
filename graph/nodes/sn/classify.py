...
class SNClassification(BaseModel):
    intent: str = Field(
        description=(
            ...
            "search_by_keyword, acknowledge_incident, add_work_notes, assign_incident, "
            "escalate_priority, update_status, resolve_incident, link_related_incidents, "
            "help, clarify, greeting, force_show"
        )
    )
    ...
    parent_incident_id: Optional[str] = Field(
        default=None,
        description="Parent INC number when linking incidents (e.g. 'link INC001 as child of INC002' -> parent=INC002)"
    )
    child_incident_id: Optional[str] = Field(
        default=None,
        description="Child INC number when linking incidents (e.g. 'link INC001 as child of INC002' -> child=INC001)"
    )
...
1. CLASSIFY the message into exactly one intent:
   ...
   ACTION intents — acknowledge_incident, add_work_notes, assign_incident, escalate_priority, update_status, resolve_incident, link_related_incidents
   ...
   Use "add_work_notes" when the user says add note, add work note, post update, or comment.
   Use "assign_incident" when the user says assign, reassign, or assign to.
   Use "escalate_priority" when the user says escalate, bump priority, or raise to P1/P2.
   Use "link_related_incidents" when the user says link, relate, or connect incidents.
...
def sn_classify_node(state):
    ...
    inc_match = re.match(r"^(INC\d+)$", question.strip(), re.IGNORECASE)
    pri_match = re.match(r"^P?([1-5])$", question.strip(), re.IGNORECASE)
    if awaiting == "priority" and pri_match:
        target_pri = int(pri_match.group(1))
        session["sn_intent"] = "escalate_priority"
        session["_turn_entities"] = {"priorities": [target_pri], **dict(session.get("accumulated_entities", {}))}
        return {"sn_session": session, "steps": ["sn_classify:awaiting_priority_injected"]}
    if awaiting == "parent_incident_id" and inc_match:
        parent_id = inc_match.group(1).upper()
        session["sn_intent"] = "link_related_incidents"
        session["_turn_entities"] = {"parent_incident_id": parent_id, **dict(session.get("accumulated_entities", {}))}
        return {"sn_session": session, "steps": ["sn_classify:awaiting_parent_injected"]}
    if awaiting == "child_incident_id" and inc_match:
        child_id = inc_match.group(1).upper()
        session["sn_intent"] = "link_related_incidents"
        session["_turn_entities"] = {"child_incident_id": child_id, **dict(session.get("accumulated_entities", {}))}
        return {"sn_session": session, "steps": ["sn_classify:awaiting_child_injected"]}
    ...
    if result.parent_incident_id:
        extracted["parent_incident_id"] = result.parent_incident_id
    if result.child_incident_id:
        extracted["child_incident_id"] = result.child_incident_id
