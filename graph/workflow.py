...
# SN nodes (4-layer architecture + action nodes)
...
def build_workflow():
    ...
    # -------------------------------------------------
    # SN CHAIN (4-layer: classify → accumulate → score → branch)
    # -------------------------------------------------

    workflow.add_edge("sn_classify", "sn_accumulate")
    workflow.add_edge("sn_accumulate", "sn_score")

    # Specificity gate: disambiguate, execute, or action flow
    workflow.add_conditional_edges(
        "sn_score",
